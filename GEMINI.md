...
-   [X] **Webui 버그 수정**: 체크메이트 시에도 엔진에게 요청을 보냄. 항상 퀸으로 프로모션함.
...
-   [ ] **오프닝 북 (Opening Book)**: `opening/eco` 파일을 파싱하여 오프닝 북을 구현함. 초반 수를 계산 없이 즉시 처리하여 성능과 안정성을 확보. [구현되고 있으나 WebUI에서 적용되지 않는 버그가 있음]
