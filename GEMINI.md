...
-   **폰 캡처 로직 수정**: a파일과 h파일에서 폰이 보드 바깥으로 캡처하는 버그 수정 완료.
...
-   **평가 함수**:
    ...
    -   **체크메이트 점수**: 체크메이트 시 매우 높은 점수를 부여하여, AI가 메이트를 우선적으로 탐색하도록 개선.
...
-   **오프닝 북 (Opening Book)**: `opening/eco` 파일을 파싱하여 오프닝 북을 구현함. 초반 수를 계산 없이 즉시 처리하여 성능과 안정성을 확보.
...
-   **WebUI 버그 수정**: 체크메이트 시 엔진에 불필요한 요청을 보내거나, 항상 퀸으로 프로모션하는 등의 WebUI 관련 버그 수정 완료.
...
### 0순위: 테스트
-   [X] **test make 통과하기**: 현재 통과함.
-   [ ] **WebUI 버그**: 흑부터 시작하는 FEN 로드시 PGN에서 흑이 먼저 오는 문제 발생. 흑부터 시작하는 FEN 로딩시 더미 PGN을 끼워넣을 것.
-   [ ] **WebUI 여러 세션 지원 / ucinewgame 또는 화면 새로고침시 엔진이 잘 응답하지 않는 버그가 있음.**
...
### 2순위: 탐색 알고리즘 고도화
-   [X] **수 정렬 (Move Ordering) 개선**: Killer Moves, History Heuristic 등 더 정교한 수 정렬 기법을 도입하여 탐색 효율을 추가로 개선.
-   [X] **Null Move Pruning**: 상대방이 한 수를 쉬게 하고 탐색을 진행하여, 위협적이지 않은 상황에서 탐색 깊이를 줄이는 가지치기 기법.
-   [ ] **Late Move Reductions (LMR)**: 탐색 후반부에 있는 수들의 탐색 깊이를 줄여, 유망하지 않은 브랜치를 빠르게 잘라내는 기법.
-   [ ] **엔드게임 분석**: 현재 체크메이트 수순을 계산하지 못함. 평가함수 / 미니맥스 진행중 체크메이트 수순 발견 시 이를 진행하도록 구현. 엔드게임 데이터베이스를 구축하여 기물이 적을 때 정확한 수순 계산 가능하도록 개선.

### 3순위: 평가 함수 정교화
-   [ ] **전략적 요소 추가**: 나이트 전초기지(Knight Outpost), 핀(Pinned)에 걸린 기물, 연결된 룩(Connected Rooks) 등 더 복잡하고 전략적인 개념들을 평가 함수에 추가.
-   [ ] **엔드게임 데이터베이스 구축**: 기물이 얼마 남지 않았을 때 체크메이트 또는 비기는 수순을 계산. [진행중 - 엔드게임에서 탐색 깊이를 늘리는 방식으로 1차 개선]
-   [ ] **동형 반복 고려**
...
코드베이스 전체를 검토한 결과, 다음과 같은 개선점을 발견했습니다. 향후 리팩토링 시 우선적으로 고려할 수 있습니다.

-   **코드 중복**: `MoveGen.cpp`와 `Bitboard.cpp`에 폰의 공격 및 캡처 관련 로직이 중복으로 구현되어 있습니다. 이를 별도의 유틸리티 함수로 통합하여 중복을 제거하고 일관성을 높일 수 있습니다.

-   **기동성 평가 비효율**: 현재 기동성 평가는 양측의 모든 수를 생성하여 계산하므로 비효율적입니다. 수를 생성하지 않고 기물의 움직임 가능성만으로 기동성을 근사 계산하는 더 가벼운 방식의 도입을 고려할 수 있습니다.

-   **SAN 변환 로직**: `Book.cpp`의 `sanToMove` 함수는 모든 수를 생성하여 비교하므로 비효율적입니다. SAN 문자열을 직접 파싱하는 방식으로 변경하면 성능을 향상시킬 수 있지만, 구현의 복잡성이 증가합니다.

-   **매직 넘버**: 평가 함수 및 비트보드 연산에 의미를 알기 어려운 숫자(매직 넘버)가 많이 사용됩니다. 가독성 향상을 위해 이름 있는 상수로 치환해야 합니다.
