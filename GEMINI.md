...
-   **평가 함수(`AIPlayer.cpp`) 복잡성**: `evaluate` 함수가 매우 길고 복잡하여 가독성과 유지보수성이 떨어집니다. 기물 가치, 위치 점수, 킹 안전성 등 각 평가 요소를 별도의 함수로 분리하여 구조를 개선할 필요가 있습니다.
...
-   **함수 중복**: `DataTypes.h`의 `Move::toString`과 `Engine.cpp`의 `moveToString` 함수가 중복됩니다. `Move::toString`을 일관되게 사용하도록 통일해야 합니다.
